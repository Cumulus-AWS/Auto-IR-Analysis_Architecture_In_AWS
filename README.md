...
### 프로젝트 필요성
**1. AWS 침해사고 증가**  
AWS를 대상으로 한 침해사고의 수와 파급력이 증가하고 있다. 일례로 2019년 발생한 Capital One의 고객정보 해킹 사건이 있다. Capital one 사건은 AWS 클라우드 환경에서 발생한 침해사고로, 해커가 AWS EC2 인스턴스에 악의적인 요청을 보내어 임시 로그인 자격 증명을 획득하였다.   
...
많은 기업들이 보안 서비스와 아키텍쳐를 이용하고 있지만, 서비스에서 사용되는 프레임워크 취약점은 예방하기 어렵다.  
대표적인 사례로 Log4shell취약점이 있다.  
LOG4J 프레임워크에서 발생된 이 취약점으로 많은 기업 서버들이 RCE공격을 당했다.  
예방이 어려운 만큼, 보다 빠른 침해사고 대응이 필요하다.  

**3. 기업의 클라우드 마이그레이션**  
"배달의 민족"서비스를 운영하는 푸드테크 기업 "우아한 형제들"은 4년간의 준비 끝에 AWS라는 클라우드 서비스로 모든 서비스를 마이그레이션하였다.  
클라우드환경은 온프레미스 환경에 비해 유연한 인프라관리, 가용성등 다양한 장점을 지니고 있다.  
...
**5. 관련 법안 준수의 필요성**  
대한민국 클라우드컴퓨팅 발전 및 이용자 보호에 관한 법률 제23조에 따르면 클라우드 컴퓨팅 서비스 제공자는 클라우드 컴퓨팅 품질∙성능 및 정보보호 수준을 향상시키기 위하여 노력하여야 한다.  
 국내 뿐 아니라, 서비스 국가의 법안을 준수하기 위해 클라우드 침해사고의 분석을 통한 보안 수준, 복원력 향상이 필요하다.
...
실제 서비스는 Server_VPC에서 이루어지며, 사용자에게 웹 애플리케이션을 서비스한다. Guardduty를 통해서 침해사고가 탐지되면 Stepfuction으로 구성된 아티팩트 채증 자동화 시스템이 작동한다. 그리고 채증된 아티팩트를 Analysis_VPC에서 분석하여 시스템의 취약점과 문제점 등을 파악하고 로그 및 아티팩트를 분석하여 네트워크 구조를 파악한다. 또한 사용자 및 장치의 활동을 추적하여 비정상적인 활동이나 침입 탐지를 분석하고, 이는 추후 잠재적 위협에 대비하는 결과를 도출한다. 
...
**4. 채증한 아티팩트를 S3에 전송**  
채증한 휘발성 및 비휘발성 아티팩트들을 S3 버킷에 아카이빙하여 분석 VPC에서 사용할 수 있도록 해야 한다. 이를 위해 채증할 아티팩트를 쌓아 둘 S3 버킷을 생성하고 AWS CLI 명령어를 통해 해당 S3로 전송되도록 설정한다. S3에 쌓인 아티팩트들을 분석 VPC에서 사용할 수 있도록 IAM Role을 통한 액세스 권한을 설정하고 VPC 엔드포인트를 설정한다.
...
**1. 분석용 EC2의 AMI 생성**  
분석 환경을 구성하기 위해서는 먼저 private 환경에서 작업하기 위해 public subnet에 EC2 인스턴스를 생성한다. 이 EC2 인스턴스에는 Volatility3, python3, YARA 등 분석에 필요한 도구와 파일들을 설치하고 구성한다. 분석 EC2를이러한 도구들을 구성한 후, AMI(Amazon Machine Image)를 생성한다. 이는 현재의 EC2 인스턴스 상태를 저장하는 이미지로써, 추후에 동일한 분석 환경을 쉽게 재현할 수 있도록 해준다. 주기적으로 업데이트된 이미지를 생성하여 보관하면, 새로운 도구나 버전 업데이트 등을 적용할 수 있다. ---뭔가 말이 수정필요 

<천웅 버젼> - 내생각에는 private subnet이 필요한 이유와 프라이빗 서브넷위에 EC2를 띄우다보니 AMI로 버젼관리를 할수 있다로 두개를 나누는게 좋을 듯?  
뭔가 저위에는 IR AMI를 말하는지 분석용 AMI를 말하는지 띵킹하기 어려움  
  
**1. 격리된 Analysis_VPC**  
침해사고 발생으로 채집된 AMI를 다시 EC2에 올려 실행하게 되면, 2차 피해가 일어날 수 있다. 예를 들어, 리버스쉘 Crontab이 설정되어 있다면, 다시 리버스쉘이 맺어질 수도 있다. 이를 방지하기 위해 SSM 엔드포인트와 S3 엔드포인트를 제외한 모든 아웃바운드가 차단된 서브넷 위에 AMI를 복원해야한다. AMI의 무결성 보존을 위해 침해사고 당한 AMI뉘에 직접 분석을 돌리는 것이 아니라 AMI을 이용해 EBS Volume을 만들고, 이를 분석용 EC2에 Mount하여 분석용 EC2에서 분석환경을 제공할 수 있도록하였다. 
  
**2. 분석용 EC2**
인/아웃바운드가 차단된 Subnet 위에서 분석환경을 구성하기 위해서는, 퍼블릭 Subnet위에 구성된 EC2에서 Volatility3, python3, YARA 등 분석에 필요한 도구와 파일들을 설치하여야한다. 따라서 이를 AMI로 본떠 Private Subnet위에 구축되어있는 분석용 EC2에 적용하면, 안전하게 Private Subnet위에서 분석을 진행할 수 있고, 새로운 도구나 버전 업데이트 등을 적용할 수 있다.  
...
**4. 분석결과를 S3에 전송**  
분석한 휘발성 및 비휘발성 아티팩트들의 결과를 추가 분석 및 백업하기 위해S3 버킷에 아카이빙한다. 이를 위해 분석 결과를 저장할 S3 버킷을 생성하고 AWS CLI 명령어를 통해 해당 S3로 전송되도록 설정한다. S3는 EC2 / 침해사고 탐지 시간으로 구조화 하였다. ----? 뭔가 이상함 
