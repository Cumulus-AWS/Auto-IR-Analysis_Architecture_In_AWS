...
**1. 분석 EC2의 AMI 생성**  
분석 환경을 구성하기 위해서는 먼저 private 환경에서 작업하기 위해 public subnet에 EC2 인스턴스를 생성한다. 이 EC2 인스턴스에는 Volatility3, python3, YARA 등 분석에 필요한 도구와 파일들을 설치하고 구성한다. 이러한 도구들을 구성한 후, AMI(Amazon Machine Image)를 생성한다. 이는 현재의 EC2 인스턴스 상태를 저장하는 이미지로써, 추후에 동일한 분석 환경을 쉽게 재현할 수 있도록 해준다. 주기적으로 업데이트된 이미지를 생성하여 보관하면, 새로운 도구나 버전 업데이트 등을 적용할 수 있다.
...
**4. 분석한 아티팩트를 S3에 전송**  
분석한 휘발성 및 비휘발성 아티팩트들의 결과를 추가 분석 및 백업하기 위해S3 버킷에 아카이빙한다. 이를 위해 분석 결과를 저장할 S3 버킷을 생성하고 AWS CLI 명령어를 통해 해당 S3로 전송되도록 설정한다. S3는 EC2 / 침해사고 탐지 시간으로 구조화 하였다.
